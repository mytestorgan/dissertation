...
Необходимо сформировать такой сигнал, который способен уравновесить действие помехового воздействия $n^*(t)\approx-n(t)$, чтобы вычесть его из наблюдаемой реализации:
...
\path (m11) node[yshift=0.5cm] {$u(k)=t_p\times \hat W(k+1)$} ;
	...
	\draw[dspconn] (m35) -- (m25);
        ...
        \end{tikzpicture}
\caption{Алгоритм управления буфером компенсации джиттера с аддитивной коррекцией}
\label{fig:man_2}
\end{figure}

Аналогично предыдущему алгоритму построим управление буфером компенсации джиттера с мультипликативной коррекцией (рис. \ref{fig:man_3})
...
\begin{enumerate}
 ...
 \item Предложено две схемы управления алгоритмом компенсацией джиттера, обеспечивающих возможность использования для аддитивной и мультипликативной коррекции фазы джиттера.
